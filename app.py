...
if prompt:
    ...
    with st.chat_message("assistant", avatar=bot_avatar):
        with st.spinner("Yazıyor..."): # "Düşünüyor" yerine "Yazıyor" daha insancıl
            try:
                
                history = [
                    {"role": "user" if m["role"] == "user" else "model", "parts": [m["content"]]}
                    for m in st.session_state.messages[:-1] # Son mesaj hariç hepsini al
                ]
                ...
                if response and response.text:
                    ...
                    st.warning("Bir sorun oluştu, tekrar dener misin?")
